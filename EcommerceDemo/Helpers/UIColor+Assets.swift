// swiftlint:disable object_literal
import UIKit

extension UIColor {
    static let darkBackground = UIColor(red: 0.15, green: 0.15, blue: 0.15, alpha: 1)
    static let darkText = UIColor(red: 0.153, green: 0.153, blue: 0.153, alpha: 1)
    static let normalText = UIColor(red: 0.153, green: 0.153, blue: 0.153, alpha: 0.4)
    static let blackProduct = UIColor(red: 0.25, green: 0.25, blue: 0.25, alpha: 1)
    static let blueProduct = UIColor(red: 0.28, green: 0.65, blue: 0.97, alpha: 1)
    static let greenProduct = UIColor(red: 0.19, green: 0.57, blue: 0.05, alpha: 1)
    static let tabBarItem = UIColor.white
    static let tabBarItemSelected = UIColor(red: 0.992, green: 0.38, blue: 0.004, alpha: 1)
    static let pageIndicator = UIColor.white
    static let pageIndicatorActive = UIColor(red: 0.992, green: 0.38, blue: 0.004, alpha: 1)
    static let separator = UIColor(red: 0.592, green: 0.592, blue: 0.592, alpha: 0.3)
}
